...
//}
    ...
    impl ZTM {
        ...
        fn get_message(&self, body: Response, date_time : &chrono::DateTime<chrono::Local>) -> String {
            self.busses.iter().for_each(|e| {
                body.departures.iter().filter(|d| d.routeId == *e ).fold(e.to_string()+":", |response, d| {
                    // Compute estimated time of arrival in minutes
                    //TODO parse estimatedTime
                    let estimation = match(d.delayInSeconds) {
                        Some(secs) => date_time.clone() + chrono::Duration::seconds(secs as i64),
                        None  => date_time.clone(),
                    };
                    let remaining_minutes = estimation.signed_duration_since(*date_time).num_minutes();
        ...
        pub fn submit(&self) -> Result<String, String> {
            ...
            let message = self.get_message(res.json::<Response>().expect("Error converting response to JSON in ZTM"), &chrono::Local::now());
            Ok(message)
        }
    }
    ...
    mod tests {
        ...
        fn test_parsing_ztm_message() -> GenericResult<()> {
            ...
            let date_str = "2022-09-07 07:25:00";
            let datetime = chrono::DateTime::parse_from_str(date_str, "%Y-%m-%d %H:%M:%S").unwrap();

            let response = ZTM::new(
                None,
                "1752", // ID of bus stop
                vec![158],// bus numbers we are interested in
